  ...
  return (
    <>
      ...
      <div className="p-6 bg-gradient-to-b from-white to-purple-50">
        <h4 className="text-2xl font-serif font-bold text-purple-900 mb-2">{product.name}</h4>
        <p className="text-purple-600 text-lg mb-3">{product.desc}</p>
          ...
          <div className="relative max-w-6xl w-full max-h-[90vh] flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
            ...
            {sortedImages.length > 1 && (
              <>
                <button 
                  onClick={(e) => { e.stopPropagation(); prevImage(); }}
                  className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-purple-900 rounded-full p-4 shadow-xl hover:scale-110 transition-all font-bold text-2xl"
                >
                  ←
                </button>
                <button 
                  onClick={(e) => { e.stopPropagation(); nextImage(); }}
                  className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-purple-900 rounded-full p-4 shadow-xl hover:scale-110 transition-all font-bold text-2xl"
                >
                  →
                </button>
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-purple-900/80 text-white px-4 py-2 rounded-full text-sm font-bold">
                  {currentImageIndex + 1} / {sortedImages.length}
                </div>
              </>
            )}
